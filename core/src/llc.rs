...
impl<'a> Alloc<'a> for LLC {
    ...
    fn get(&self, core: usize, frame: Option<usize>, flags: Flags) -> Result<usize> {
        let ret = if let Some(frame) = frame {
            unsafe {
                bindings::llfree_get_at(self.raw.get().cast(), core as _, frame as _, flags.into())
            }
        } else {
    ...
    fn put(&self, core: usize, frame: usize, flags: Flags) -> Result<()> {
        let ret = unsafe {
            bindings::llfree_put(self.raw.get().cast(), core as _, frame as _, flags.into())
        };
        ret.ok().map(|_| ())
    }
    ...
    fn is_free(&self, frame: usize, order: usize) -> bool {
        let stats = unsafe {
            bindings::llfree_full_stats_at(self.raw.get().cast(), frame as _, order as _)
        };
    ...
    fn fast_stats_at(&self, frame: usize, order: usize) -> crate::Stats {
        unsafe { bindings::llfree_stats_at(self.raw.get().cast(), frame as _, order as _).into() }
    }
    ...
    fn stats_at(&self, frame: usize, order: usize) -> crate::Stats {
        unsafe {
            bindings::llfree_full_stats_at(self.raw.get().cast(), frame as _, order as _).into()
        }
    }
