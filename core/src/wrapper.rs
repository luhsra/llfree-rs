...
impl<'a, A: Alloc<'a>> Alloc<'a> for ZoneAlloc<'a, A> {
    ...
    fn new(frames: usize, init: Init, classing: &Classing, meta: MetaData<'a>) -> Result<Self> {
        Ok(Self {
            alloc: A::new(frames, init, classing, meta)?,
            offset: 0,
            _p: PhantomData,
        })
    }
    ...
    fn metadata_size(classing: &Classing, frames: usize) -> MetaSize {
        A::metadata_size(classing, frames)
    }
    ...
    fn get(&self, frame: Option<FrameId>, flags: Request) -> Result<(FrameId, Class)> {
        let frame = frame
            ...
            .transpose()?;
        let (frame_id, class) = self.alloc.get(frame, flags)?;
        Ok((FrameId(frame_id.0 + self.offset), class))
    }
...
impl<'a, A: Alloc<'a>> NvmAlloc<'a, A> {
    pub fn create(
        ...
        let m = A::metadata_size(classing, zone.len());
...
impl<'a, A: Alloc<'a>> Alloc<'a> for NvmAlloc<'a, A> {
    ...
    fn new(_frames: usize, _init: Init, _classing: &Classing, _meta: MetaData) -> Result<Self> {
        unimplemented!()
    }
    fn metadata_size(classing: &Classing, frames: usize) -> MetaSize {
        A::metadata_size(classing, frames)
