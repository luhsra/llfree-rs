...
- **Upper level (`core/src/llfree.rs`)**
  ...
  - Uses per-class local reservations (`Locals`) to reduce contention.
  - Uses global tree metadata (`Trees`) updated via atomics/CAS.
  - Allocation fallback order: match -> demote -> class downgrade -> steal.

- **Classing/policy (`core/src/lib.rs`)**
  - `Classing` defines classes + local-slot counts + policy function.
  - `Policy` values: `Match(priority)`, `Steal`, `Demote`, `Invalid`.
  - Built-in examples: `classing::simple`, `classing::movable`.

- **Key invariants**
  - Tree counters stay consistent with lower-level state.
  - Reserved trees are not mutated by unsupported paths.
  - Class stats (`TreeStats`) reflect tree transitions.
